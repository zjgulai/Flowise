...
-   当前边界：PR `zjgulai/Flowise#10` 已包含第三笔 seccomp metadata/CI review commit `bbc4b444264300b01b8dadcb2ce956aeee656eb7`；该 head 的 Docker CI 与 CodeRabbit 已通过，Node CI 暴露的既有 Chatflow Cypress alias 竞态已在第四笔 test-only 候选中关闭，本地双轮 Chrome 与独立复审均通过，待原子提交。PR 尚未 merge，仍未生成包含 recovery 命令的自绑定 artifact，也未对生产执行 recovery、cutover 或浏览器验收；`production_write=false`（本恢复批次开始以来）。
-   下一门禁：原子提交 Chatflow Cypress 稳定性修复 -> 新 exact-head CI/merge -> main 手工 readiness artifact -> production recovery terminal closure -> 新版本 cutover -> PC-first browser acceptance -> cleanup。
