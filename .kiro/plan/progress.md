...
-   当前边界：PR `zjgulai/Flowise#10` 已包含前两笔原子 commit；第三笔 seccomp metadata/CI review 修复仍待 commit、push 与新 head CI，PR 尚未 merge。仍未生成包含 recovery 命令的自绑定 artifact，也未对生产执行 recovery、cutover 或浏览器验收；`production_write=false`（本恢复批次开始以来）。
...
-   新 owner allowlist 使 macOS UID 501 的真实 Docker fixture 首次按预期失败且 trap 清理为零残留；仅在 BoundaryHarness 内加入 scoped unprivileged reader，并以 observation 前固定的 root FD、逐组件 `dir_fd + O_DIRECTORY + O_NOFOLLOW` 与末级前后 identity 关闭父路径竞态。最终 SHA 重跑真实 Docker `8/8`、fixture residue `0`；生产 wrapper owner allowlist 未放宽。下一步提交第三个原子修复 commit 并等待新 head CI。
