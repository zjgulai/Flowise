...
if docker run --detach \
    ...
    --tmpfs /usr/src/flowise/.flowise:rw,uid=1000,gid=1000,mode=0700 \
    --tmpfs /usr/src/flowise/packages/server/logs:rw,uid=1000,gid=1000,mode=0700 \
...
docker inspect --format '{{json .HostConfig.SecurityOpt}}' "$SMOKE_NAME" | grep -Eq '^\["no-new-privileges(:true)?"\]$' || fail 'isolated runtime privilege escalation guard changed'
