...
file_fixed_count "packages/components/package.json" '"js-yaml": "4.3.1"' 1 "Components declares the OpenAPI Toolkit runtime YAML dependency"
...
file_fixed_count "Dockerfile" "/var/log/apk.log" 2 "Dockerfile removes the nondeterministic APK log in both independent stages"
file_fixed_count "Dockerfile" "node_modules/.modules.yaml" 1 "Dockerfile removes pnpm's dynamically timestamped modules state before the runtime copy"
file_fixed_count "Dockerfile" "node_modules/.pnpm-workspace-state-v1.json" 1 "Dockerfile removes pnpm's dynamically timestamped workspace state before the runtime copy"
file_fixed_count "Dockerfile" "        .turbo \\" 1 "Dockerfile removes the root Turbo state before the runtime copy"
...
file_fixed_count ".github/workflows/test_docker_build.yml" "git clean -dffqx" 1 "Docker test CI resets the primary build to one canonical ignored-file-free context"
file_fixed_count ".github/workflows/test_docker_build.yml" 'test -z "$(git status --porcelain --untracked-files=all)"' 3 "Docker build and readiness CI prove tracked and untracked context cleanliness"
file_fixed_count ".github/workflows/test_docker_build.yml" 'test -z "$(git clean -ndffx)"' 2 "Docker build and readiness CI prove no ignored context residue remains"
file_contains_in_order ".github/workflows/test_docker_build.yml" 'test -z "$(git status --porcelain --untracked-files=all)"' "git clean -dffqx" "Docker test CI proves the source tree is clean before canonical context reset"
file_contains_in_order ".github/workflows/test_docker_build.yml" "git clean -dffqx" 'test -z "$(git clean -ndffx)"' "Docker test CI proves canonical context reset removed all ignored residue"
...
file_fixed_count ".github/workflows/docker-image-dockerhub.yml" "git clean -dffqx" 1 "Docker Hub CI resets the publish build to one canonical ignored-file-free context"
file_fixed_count ".github/workflows/docker-image-dockerhub.yml" 'test -z "$(git status --porcelain --untracked-files=all)"' 2 "Docker Hub CI proves source cleanliness before and after context reset"
file_fixed_count ".github/workflows/docker-image-dockerhub.yml" 'test -z "$(git clean -ndffx)"' 1 "Docker Hub CI proves no ignored context residue remains"
file_contains_in_order ".github/workflows/docker-image-dockerhub.yml" 'test -z "$(git status --porcelain --untracked-files=all)"' "git clean -dffqx" "Docker Hub CI proves the source tree is clean before canonical context reset"
file_contains_in_order ".github/workflows/docker-image-dockerhub.yml" "git clean -dffqx" 'test -z "$(git clean -ndffx)"' "Docker Hub CI proves canonical context reset removed all ignored residue"
...
file_contains ".github/workflows/test_docker_build.yml" 'REBUILD_TAG: flowise-chinese:git-${{ github.sha }}' "Release readiness rebuild uses the canonical production image tag"
file_fixed_count ".github/workflows/test_docker_build.yml" "--file Dockerfile" 1 "Release readiness rebuild explicitly selects the canonical root Dockerfile"
file_contains ".github/workflows/test_docker_build.yml" 'release_readiness_config_mismatch expected=%s actual=%s' "Release readiness emits only the expected and actual config digests on mismatch"
file_fixed_count ".github/workflows/test_docker_build.yml" 'test "$independent_config_digest" = "$expected_config_digest"' 1 "Release readiness requires exactly one raw independent-to-primary config identity equality gate"
file_not_contains_regex ".github/workflows/test_docker_build.yml" 'test "\$independent_config_digest" = "\$expected_config_digest"[[:space:]]*(\|\||;[[:space:]]*true)' "Release readiness cannot bypass the raw config identity equality gate"
file_contains_in_order ".github/workflows/test_docker_build.yml" 'release_readiness_config_mismatch expected=%s actual=%s' 'test "$independent_config_digest" = "$expected_config_digest"' "Release readiness reports a safe digest-only mismatch summary before failing closed"
...
file_contains "scripts/verify-release-candidate.sh" '[[ "$store_identity" == "$image_config_digest" ]]' "Release evidence binds Docker store and archive config identities"
...
file_contains "scripts/flowise-production-release.py" 'EXPECTED_BUILDX_VERSION = "v0.34.1"' "Production wrapper binds the Buildx evidence"
file_contains "scripts/flowise-production-release.py" 'EXPECTED_BUILDKIT_VERSION = "v0.30.0"' "Production wrapper binds the BuildKit evidence"
file_fixed_count "scripts/flowise-production-release.py" '"--tmpfs",' 2 "Migration inventory exposes exactly two bounded tmpfs write surfaces"
file_contains "scripts/flowise-production-release.py" '"/home/node/.flowise/storage:rw,noexec,nosuid,nodev,size=8m,nr_inodes=2048,uid=1000,gid=1000,mode=0700"' "Migration inventory bounds its storage scratch leaf"
file_contains "scripts/flowise-production-release.py" '"/usr/src/flowise/packages/server/logs:rw,noexec,nosuid,nodev,size=8m,nr_inodes=2048,uid=1000,gid=1000,mode=0700"' "Migration inventory bounds its logger scratch leaf"
file_not_contains_regex "scripts/flowise-production-release.py" '"/home/node/\.flowise:rw' "Migration inventory does not widen scratch access to the Flowise home directory"
file_contains "scripts/flowise-production-release.py" 'TRANSITION_PERMITS_DIR = BASE_DIR / "transition-permits"' "Production wrapper fixes the transition permit outside deployment journals"
file_contains "scripts/flowise-production-release.py" 'commands.add_parser("snapshot-transition")' "Production wrapper exposes the read-only transition snapshot command"
file_contains "scripts/flowise-production-release.py" 'commands.add_parser("issue-transition-permit")' "Production wrapper exposes the digest-gated transition permit issuer"
file_contains "scripts/flowise-production-release.py" 'issue_parser.add_argument("--expected-snapshot-sha256", required=True, type=_digest_argument)' "Transition permit issuance requires an exact snapshot digest"
file_contains "scripts/flowise-production-release.py" "permit_document=initial_permit" "Transition snapshot reobserves through the permit consumer binding"
file_contains "scripts/flowise-production-release.py" "os.link(temporary, destination, follow_symlinks=False)" "Transition permit publication is atomic and no-overwrite"
file_contains "scripts/flowise-production-release.py" "os.fchmod(descriptor, 0o000)" "Failed transition permit publication creates a durable unreadable tombstone"
file_contains "scripts/flowise-production-release.py" "LEGACY_BOOTSTRAP_REPOSITORY_URLS = (" "Legacy transition permits accept only canonical repository provenance"
file_not_contains_regex "scripts/flowise-production-release.py" "allow_self_bound" "Production wrapper has no self-authorizing bootstrap permit bypass"
file_contains "scripts/flowise-production-release.py" 'bootstrap_parser.add_argument("--transition-permit", required=True, type=Path)' "Bootstrap requires an explicit transition permit"
file_contains "scripts/flowise-production-release.py" 'bootstrap_parser.add_argument("--transition-permit-sha256", required=True, type=_digest_argument)' "Bootstrap requires the exact transition permit digest"
file_contains "scripts/flowise-production-release.py" "permit = verify_transition_permit(" "Bootstrap verifies the explicit permit before transition"
file_contains "docs/ops/flowise-legacy-hardening-transition-20260727.md" "snapshot-transition" "Legacy transition runbook documents the supported snapshot command"
file_contains "docs/ops/flowise-legacy-hardening-transition-20260727.md" "issue-transition-permit" "Legacy transition runbook documents the supported permit issuer"
