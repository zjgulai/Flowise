...
-   [ ] **Step 1: Review the candidate UI diff for executable-identifier translation and mixed concerns**
...
-   [ ] **Step 2: Repair executable-identifier, behavior, selector, and copy blockers before staging**
...
-   [ ] **Step 3: Stage, verify, and commit exactly one sub-unit at a time**
...
-   Produces: the existing Node 24 multi-stage image/build baseline, clean-workspace dependency layout, direct non-root runtime entrypoint, and UI resolver/dependency compatibility fixes.

-   [ ] **Step 1: Keep this commit limited to image/build inputs**

Keep Compose, env template, cross-cutting verification scripts, Provider configuration, CSP configuration, release-manifest/OCI labels, and exact toolchain changes for Task 6. Stage only the existing Node 24 multi-stage build, clean-workspace dependency layout, direct Node/Oclif runtime, dependency pins, and Vite resolver fix.

-   [ ] **Step 2: Review lockfile ownership**

Confirm each staged importer change corresponds to `packages/ui/package.json`. Do not accept unrelated version upgrades. If pnpm regenerates the lockfile, use Node `24.18.0` and pnpm `10.26.0`, then compare package resolutions before staging.
...
Expected: strict install, UI build, and the currently applicable static gate pass without provider calls. Compose/env-specific assertions remain owned by Task 6.
...
-   Produces: required credentials, official-origin allowlists, redirect-aware secure fetch, deterministic timeout/retry behavior, and current DeepSeek/Kimi model metadata.

-   [ ] **Step 1: Stage Provider-only code/config/docs**

Do not stage CSP, unrelated Docker, root provider guides, real credentials, or compiled output.

-   [ ] **Step 2: Run Provider L2 gates**

Run:

```bash
pnpm --filter flowise-components exec jest nodes/chatmodels/providerUtils.test.ts nodes/chatmodels/Deepseek/Deepseek.test.ts nodes/chatmodels/ChatKimi/ChatKimi.test.ts nodes/chatmodels/ProviderCatalog.test.ts --runInBand
pnpm --filter flowise-components exec tsc --noEmit
pnpm --filter flowise-components build
git diff --cached --check
```

Expected: mock/local tests and build pass; `provider_call=false` and `secrets_read=false` remain true.
...
-   Produces: controlled auth method boundary, iframe-origin fail-fast, structured CSP modes, bounded sanitized report receiver, and same-origin UI bootstrap.

-   [ ] **Step 1: Stage only request/CSP/security hunks**

Keep Chinese metadata already owned by Task 2 and release-provenance additions for Task 6 out of this staged set.

-   [ ] **Step 2: Run CSP/request gates**

Run:

```bash
pnpm --filter flowise exec jest --runInBand src/utils/XSS.test.ts src/utils/csp.test.ts src/utils/cspReport.test.ts src/services/chatflows/index.test.ts
...
-   [ ] **Step 1: Write failing release-manifest unit tests**
...
-   [ ] **Step 3: Implement the minimal generator/verifier**
...
-   [ ] **Step 4: Add failing static release-contract checks**
...
-   [ ] **Step 5: Implement exact toolchain, Docker, Compose, and CI config**
...
-   [ ] **Step 1: Update only verified current facts**
...
-   [ ] **Step 2: Correct stale routing statements**
...
-   [ ] **Step 3: Keep historical root reports out of the release**
...
-   [ ] **Step 4: Verify docs and commit Task 7**

```bash
rg -n "171 tracked|27 untracked|public.*3000.*accessible|bb773ffa710bd4dd" AGENTS.md .kiro docs
git diff --check
git diff --cached --check
```

Expected: no stale current-state matches outside clearly labeled historical baseline text.
